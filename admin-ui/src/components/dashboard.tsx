...
export function Dashboard({ onLogout }: DashboardProps) {
  ...
  const balancesQueriedOnce = useRef(false)
  ...
  // 添加/删除账号后自动拉取新账号余额
  useEffect(() => {
    if (!data?.credentials || !balancesQueriedOnce.current || isFetchingBalances.current) return
  ...
  useEffect(() => {
    if (!balancesQueriedOnce.current || isFetchingBalances.current) return
  ...
  const handleQueryCurrentPageInfo = async () => {
    ...
    balancesQueriedOnce.current = true
    isFetchingBalances.current = true
